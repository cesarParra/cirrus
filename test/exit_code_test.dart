...
void main() {
  ...
  void withConfig(String toml, {CliRunner? runner}) {
    getIt.registerSingleton<Either<String, Config>>(
      Right(Config.parse(TomlDocument.parse(toml).toMap())),
    );
  ...
  test('is non-zero when the config file cannot be read', () async {
    getIt.registerSingleton<Either<String, Config>>(
      Left('Was not able to load the cirrus.toml file.'),
    );
